    ...
    <div className="page page-event">
      ...
      {activeTab === "seating" && (
        <div className="seating-view">
          ...
          {/* Empty-state overlay rendered on top of the (now dimmed)
              seating view when no schedule item is marked "Requires
              seating." The dimmed chart underneath stays visible so
              the user can see what they're missing; the overlay's
              `pointer-events: auto` intercepts all clicks so the
              toolbar / canvas / sidebar can't be poked at until a
              seating event exists. */}
          {arrangements.length === 0 && (
            <div className="seating-empty-state-overlay">
